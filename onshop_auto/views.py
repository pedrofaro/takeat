...
from .models import CategoriaAuto, AtributoAuto, ProdutoAuto, PerguntaAuto, RespostaAuto, ComplementoModeloAuto, PerguntaModeloAuto, RespostaModeloAuto, Mesa, Codigo, Comanda, ComandaSession, PedidoAuto, ProdutoPedidoAuto, CompradorPedidoAuto, MensagemAuto, ItemPedidoAuto, QAvaliado, Avaliacao, Garcom
from .forms import CategoriaForm, ProdutoForm, PerguntaForm, RespostaForm, ComplementoModeloForm, PerguntaModeloForm, RespostaModeloForm, AtribuirForm, AtributoForm, MesaForm, CodigoForm, ProdutoMobileForm, ContatoForm, MensagemAutoForm, MesaAbertaForm, MesasForm, AvaliacaoForm, NotaAvaliacaoForm, GarcomForm, AtendimentoForm
...
def painel_produtos_auto(request):
	...
	return render(request, 'onshop_auto/painel_produtos_auto.html', {'produtos':produtos, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens })
...
def ver_categorias(request):
	...
	return render(request, 'onshop_auto/categorias_auto.html', {'categorias':categorias, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens })
...
def ver_atributos(request):
	...
	return render(request, 'onshop_auto/atributos.html', {'atributos':atributos, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens})
...
def ver_mesas(request):
	...
	return render(request, 'onshop_auto/mesas.html', {'mesas': mesas, 'pedidos':pedidos, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens})
...
def ver_mesas_abertas(request):
	...
	return render(request, 'onshop_auto/mesas_abertas.html', {'mesas': mesas, 'pedidos':pedidos, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens})
...
def ver_detalhes_mesa_aberta(request, id_mesa):
	...
	return render(request, 'onshop_auto/mesa_aberta_detalhes.html', {'mesa': mesa, 'pedidos':itens_pedidos, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens})
...
def ver_ultimas_mesas_fechadas(request):
	...
	'''
	comandas = Comanda.objects.filter(status=Comanda.COMANDA_FECHADA).order_by('-id')
	...
	return render(request, 'onshop_auto/mesas_fechadas.html', {'comandas': comandas, 'pedidos':pedidos, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens})
...
def acessar_autoatendimento(request):
	...
	'''
	try:
		comandasession = get_object_or_404(ComandaSession, session_key=request.session.session_key, comanda__status=Comanda.COMANDA_ABERTA)
...
def registrar_codigo(request):
	...
	if request.method == 'POST':
		...
		if codigo_form.is_valid():
			...
			try:
				comanda = get_object_or_404(Comanda, mesa=mesa, status=Comanda.COMANDA_ABERTA) #Tenta encontrar uma comanda em aberto da Mesa
			except:
				comanda = Comanda.objects.create(mesa=mesa, status=Comanda.COMANDA_ABERTA, hora_abertura=datetime.now(), total=Decimal(0.0)) #Caso contrário abre uma comanda da Mesa

			try:
				comandasession = get_object_or_404(ComandaSession, session_key=request.session.session_key, comanda=comanda) #Caso já exista a ComandaSession não faz nada
			except:
...
def ver_cardapio(request, id_comanda=None): #O None é só para a função ser aceita quando é chamada por Ajax
	...
	try:
		comandasession = get_object_or_404(ComandaSession, session_key=request.session.session_key, comanda__status=Comanda.COMANDA_ABERTA, comanda__id=id_comanda)
	except:
	...
	try: 
		pedido = get_object_or_404(PedidoAuto, session_key=request.session.session_key, status=PedidoAuto.PEDIDO_CARRINHO)
		quantidade = pedido.quantidade_itens
		total = pedido.total
	except:
		pedido = PedidoAuto.objects.create(comanda=comandasession.comanda, session_key=request.session.session_key, status=PedidoAuto.PEDIDO_CARRINHO)
		quantidade = 0
		total = 0.00
	...
	return render(request, 'onshop_auto/cardapio.html',
                          {'lista': lista, 'mesa': mesa, 'estabelecimento': estabelecimento, 'quantidade': quantidade,
                           'total': total, 'comanda': id_comanda, 'categorias' :categorias, 'token': 0, 'id_garcom': 0, 'session_key': request.session.session_key })
...
def contato(request):
	...
	try:
		comandasession = get_object_or_404(ComandaSession, session_key=request.session.session_key, comanda__status=Comanda.COMANDA_ABERTA)
		pedido = get_object_or_404(PedidoAuto, session_key=request.session.session_key, comanda=comandasession.comanda, status=PedidoAuto.PEDIDO_CARRINHO)
	except:
		return redirect ('onshop_auto:acessar_autoatendimento')

	if pedido.quantidade_itens == int(0): #Mesmo que haja uma sessão aberta, mas ainda não colocou nada no pedido, permanece no cardápio
		return redirect('onshop_auto:acessar_autoatendimento') #Se não possui pedido então não é para definir contato ainda 
	...
	return render(request, 'onshop_auto/contato.html', {'form': contato_form, 'estabelecimento': estabelecimento, 'mesa': mesa, 'quantidade': pedido.quantidade_itens, 'total': total })
...
def confirmacao(request):
	...
	return render(request, 'onshop_auto/confirmacao.html', { 'pedido':pedido, 'estabelecimento':estabelecimento, 'comandasession': comandasession, 'mesa': mesa, 'quantidade': pedido.quantidade_itens, 'total': total })
...
def concluir_pedido(request):	
	...
	return render(request, 'onshop_auto/confirmacao_modal.html', { 'pedido':pedido, 'estabelecimento': estabelecimento, 'comandasession': comandasession, 'mesa': mesa, 'quantidade': pedido.quantidade_itens, 'total': total })
...
def ver_comandas(request):
	...
	return render(request, 'onshop_auto/comandas_auto.html', {'comandas': comandas, 'pedidos':pedidos, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados})
...
def imprimir_pedido_auto(request, id):
	...
	produtos = pedido.produtos.all().order_by('produto__categoria')
	#Para pegar os pedidos por ordem de categoria
	...
	lista_produtos = agrupar_produtos_categoria(produtos)

	telefone_protegido = pedido.comprador.telefone[-4:]
	return render(request, 'onshop_auto/imprimir_pedido_auto.html', {'estabelecimento':estabelecimento, 'pedido':pedido, 'lista_produtos': lista_produtos, 'telefone_protegido': telefone_protegido})
...
def imprimir_cupom(request, id_comanda):
	...
	#Fecha a Mesa se estiver aberta também
	mesa = comanda.mesa

	if mesa.status == Mesa.MESA_ABERTA:
		#TODO: Verificar se há ainda pedidos pendentes para a Mesa
		pedidos_fluxo = PedidoAuto.objects.filter(comanda=comanda, status=PedidoAuto.PEDIDO_ANDAMENTO)

		for pedido in pedidos_fluxo: #Por enquanto joga os pedidos no limbo
	...
	taxa_servico = comanda.total * Decimal(0.1)

	dez_porcento = comanda.total * Decimal(1.1)
	return render(request, 'onshop_auto/pedido_alternativo_auto.html', {'estabelecimento':estabelecimento, 'comanda': comanda, 'pedidos':pedidos, 'dezporcento' : dez_porcento, 'taxa_servico': taxa_servico})

@login_required
def imprimir_cupom_mensagem(request, id_comanda, id_mensagem):
	'''
	Imprimi o cupom quando clica OK na mensagem de fechamento de Mesa
	'''
	comanda = get_object_or_404(Comanda, id=id_comanda)
	estabelecimento = Estabelecimento.objects.all()[0]
	estabelecimento = estabelecimento.nome
...
def imprimir_qrcode(request, id_mesa):
	...
	return render(request, 'onshop_auto/impressao_qrcode.html', {'estabelecimento':estabelecimento,'codigo':mesa.codigo_mesa, 'mesa': mesa, 'site': site})

# ------------ Gerenciamento de Itens Pedidos -----------
# ------------ -------------------
#Funções facilitadores de Mudança de Mesa e etc...

def agrupar_produtos_categoria(lista_produtos):
	#Função que retorna uma lista com a categoria e os produtos pedidos daquela categoria

	lista_aux = []
	lista_cat = []
	lista_total = []

	for aux in lista_produtos:
		if aux.produto.categoria not in lista_cat:
			lista_cat.append(aux.produto.categoria)

	ultimo = lista_cat[-1]

	for aux in lista_cat:
		lista_aux = retornaProdutos(aux, lista_produtos)
		if aux == ultimo:
			lista_total.append([aux.nome, lista_aux],)
		else:
			lista_total.append([aux.nome, lista_aux])
		'''
		for i in lista_produtos:
			if i.produto.categoria == aux:
				lista_aux.append(i)
			else:
				if aux == ultimo: #Caso seja o último
					lista_total.append([aux.nome, lista_aux])
				else:
					lista_total.append([aux.nome, lista_aux],)
				lista_aux = []
...
def retornaProdutosComprados(comprador, lista_produtos):
	lista_aux = []

	for i in lista_produtos:
		if i.comprador == comprador:
			lista_aux.append(i)

	return lista_aux

def retornaValorProdutos(lista_produtos):
	aux = Decimal(0)
...
def retornaListaDescritiva(lista_produtos): #Devolve uma lista no formato para a impresão
	lista_aux = []
	ultimo_pedido = lista_produtos[-1]

	for pedido in lista_produtos:
...
def retornaProdutos(categoria, lista):
	lista_aux = []

	for i in lista:
		if i.produto.categoria == categoria:
			lista_aux.append(i)
...
def lista_organizadora_cupom_conta(comanda):
	...
	for aux in pedidos:
		if aux.comprador not in lista_compradores_unicos:
			lista_compradores_unicos.append(aux.comprador)

	ultimo_comprador = lista_compradores_unicos[-1]

	for aux in lista_compradores_unicos:
		produtos_comprados = retornaProdutosComprados(aux, pedidos)
		valor_comprado = retornaValorProdutos(produtos_comprados)
		lista_descritiva = retornaListaDescritiva(produtos_comprados) #Retorna uma lista dentro do formato já feito para a impressão
		lista_descritiva.append(['Serviço (10%)', 0,'',None, None, valor_comprado*Decimal(0.1)])
		if aux == ultimo_comprador:
			lista_compradores.append([aux.telefone, valor_comprado*Decimal(1.1), lista_descritiva],)
		else:
			lista_compradores.append([aux.telefone, valor_comprado*Decimal(1.1), lista_descritiva])
	'''
	for pedido in pedidos:
	...
	for x in lista:
		if x[3] not in lista_compradores_unicos:
			if len(lista_compradores_unicos): #Se for o primeiro elemento não entra
				lista_aux.append(['Serviço (10%)', 0,'',None, None, valor*Decimal(0.1)],)
				lista_compradores.append([lista_compradores_unicos[-1], valor*Decimal(1.1), lista_aux],)
			valor = Decimal(0)
			lista_aux = []
...
def transferir_item_pedido(request, id_item):
	...
	return render(request, 'onshop_auto/mesa_aberta_detalhes_form.html', {'form': mesa_form, 'mesa': mesa_antiga, 'pedido':item, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados })
...
def transferir_mesa_inteira(request, id_mesa):
	...
	if request.method == 'POST':
		...
		if mesa_form.is_valid():
			...
			try:
				nova_comanda = get_object_or_404(Comanda, mesa=nova_mesa, status=Comanda.COMANDA_ABERTA) #Tenta encontrar uma comanda em aberto da Mesa
			except:
				nova_comanda = Comanda.objects.create(mesa=nova_mesa, status=Comanda.COMANDA_ABERTA, hora_abertura=datetime.now(), total=Decimal(0.0)) #Caso contrário abre uma comanda da Mesa
	...
	return render(request, 'onshop_auto/transferir_mesa_form.html', {'form': mesa_form, 'mesa': mesa, 'pedidos':itens_pedidos, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados })
...
def formulario_carrinho(request):
	...
	if token != '0': #É porque é Garçom
		...
		try:
			...
			comprador = CompradorPedidoAuto.objects.create(session_key=request.session.session_key, telefone=garcom.nome) #Para se diferenciar dos clientes da mesa é colocado o nome do Garçom
...
def ver_parcial(request):
	id_comanda = request.GET.get('id', None)
	id_garcom = request.GET.get('id_garcom', '0')
	...
	taxa_servico = Decimal(0.1) * Decimal(comanda.total)
	dezporcento = Decimal(1.1) * Decimal(comanda.total)
...
def excluir_item_pedido_carrinho(request):
	'''
	Retorna somente o valor total da comanda
	Que deve ser atualizado no botão do formulário carrinho
	'''
	pedido_id = request.GET.get('pedido_id', None)
	produtopedido_id = request.GET.get('produtopedido_id', None)
	...
	if pedido.quantidade_itens == 0:
		return JsonResponse({'product_snippet':'zero'})
	else:	
		return JsonResponse({'product_snippet':pedido.quantidade_itens, 'total': pedido.total})
	'''
	return JsonResponse({'product_snippet':pedido_id})
	'''
...
def registrar_codigo_ajax(request):
	...
	try: 
		...
		response = JsonResponse({'status':'error', 'codigo': codigo_scan, 'product_snippet': 'Esse código de mesa é inválido!'})
...
def inserir_contato(request):
	...
	pedido = get_object_or_404(PedidoAuto, session_key=key, comanda=comandasession.comanda, status=PedidoAuto.PEDIDO_CARRINHO)
...
def adicionar_pedido(request):
	...
	try: 
		pedido = get_object_or_404(PedidoAuto, session_key=request.session.session_key, status=PedidoAuto.PEDIDO_CARRINHO) #Verifica se há pedidos ainda em aberto
	except:
	...
	produtopedido = ProdutoPedidoAuto.objects.create(produto=produto, quantidade=quantidade_pp, preco_produto=preco_pp, adicional=adicional_pp, total=total_pp)
	lista_aux = []
...
def enviar_notificacao_push(request):
	...
	payload = {"app_id": app_id, 
			...
			"contents": {"en": "Foi realizado um novo pedido no AUTOATENDIMENTO!", "pt": "Foi realizado um novo pedido no AUTOATENDIMENTO!"}}
...
def enviar_notificacao_mensagem(request, eh_ajuda):
	...
	header = {"Content-Type": "application/json; charset=utf-8",
   	...
   	if eh_ajuda: #eh_ajuda 1 ou eh_ajuda 0 -> é fechamento de conta
		payload = {"app_id": app_id, 
				"included_segments": ["All"],
				"url": "/auto/painel/ver-mensagens/",
				"headings": {"en": "Mensagem de pedido de ajuda chegando", "pt": "Nova mensagem de Ajuda chegando!"},
				"contents": {"en": "Mensagem de pedido de ajuda chegando!", "pt": "Chegou uma nova mensagem de Ajuda!"}}
	else:
		payload = {"app_id": app_id, 
				"included_segments": ["All"],
				"url": "/auto/painel/ver-mensagens/",
				"headings": {"en": "Pedido de fechamento de Mesa chegando", "pt": "Pedido de fechamento de Mesa chegando!"},
...
def novos_pedidos_auto(request):
	...
	return render(request, 'onshop_auto/novos_pedidos_auto.html', { 'pedidos':pedidos, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens})
...
def pedidos_andamento(request):
	...
	return render(request, 'onshop_auto/pedidos_andamento.html', { 'pedidos':pedidos, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens})
...
def pedidos_finalizados(request):
	...
	return render(request, 'onshop_auto/pedidos_finalizados.html', { 'pedidos':pedidos, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens})
...
def ver_mensagens(request):
	...
	return render(request, 'onshop_auto/mensagens_auto.html', { 'mensagens': mensagens, 'pedidos':pedidos, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens})
...
def ver_pedido(request, id):
	try:
		pedido = get_object_or_404(PedidoAuto, status=PedidoAuto.PEDIDO_NOVO, id=id) #Se não for novo está em andamento
	except:
		pedido = get_object_or_404(PedidoAuto, status=PedidoAuto.PEDIDO_ANDAMENTO, id=id)
	...
	return render(request, 'onshop_auto/ver_pedido.html', { 'pedido':pedido, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados})

@login_required
def rejeitar_pedido(request, pedido_id=None):
	try:
		pedido = get_object_or_404(PedidoAuto, status=PedidoAuto.PEDIDO_NOVO, id=pedido_id) #Se não for novo está em andamento
	except:
...
def ver_pedido_finalizado(request, id):
	...
	return render(request, 'onshop_auto/ver_pedido_finalizado.html', { 'pedido':pedido, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados})
...
def fechar_mesa(request, id_mesa):
	...
	#TODO: Verificar se há ainda pedidos pendentes para a Mesa
	pedidos = PedidoAuto.objects.filter(comanda=comanda, status=PedidoAuto.PEDIDO_ANDAMENTO)

	for pedido in pedidos: #Por enquanto joga os pedidos no limbo
		pedido.status = PedidoAuto.PEDIDO_FINALIZADO
		pedido.save()
	
	#Mesmo com a mesa sendo fechada, os pedidos novos e em andamento precisam continuar sendo atendidos
...
def ver_avaliacoes(request):
	...
	#Para a parte de paginacao
	avaliacoes = Avaliacao.objects.all().order_by('-hora_criacao')
	page = request.GET.get('page', 1)

	paginator = Paginator(avaliacoes, 10)

	try:
		avaliacoes = paginator.page(page)
	except PageNotAnInteger:
		avaliacoes = paginator.page(1)
	except EmptyPage:
		avaliacoes = paginator.page(paginator.num_pages)

	return render(request, 'onshop_auto/avaliacoes.html', { 'q_avaliado': q_avaliado, 'form': avaliacao_form, 'pedidos':pedidos, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens, 'avaliacoes': avaliacoes })
...
def ver_avaliacao_modal(request, id_comanda):
	...
	return render(request, 'onshop_auto/avaliacao_modal.html', { 'form': avaliacao_form, 'q_avaliado': q_avaliado, 'comanda': comanda})
...
def ver_respostas(request, id_pergunta):
	...
	return render(request, 'onshop_auto/respostas.html', {'produto':produto, 'pergunta': pergunta, 'respostas': respostas})
...
def adicionar_resposta(request, id_pergunta, id_resposta=None):
	...
	return render(request, 'onshop_auto/form_resposta.html', {'produto':produto, 'pergunta': pergunta, 'form': resposta_form})
...
def ver_complementos(request):
	...
	return render(request, 'onshop_auto/lista_complementos_modelo.html', {'complementos':complementos, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens })
...
def ver_lista_complemento(request, id=id):
	...
	return render(request, 'onshop_auto/ver_lista_complementos_modelo.html', {'complemento':complemento_modelo, 'perguntas':perguntas_modelo})
...
def adicionar_pergunta_modelo(request, id, id_pergunta=None):
	...
	return render(request, 'onshop_auto/perguntas_modelo.html', {'complemento': modelo, 'modelo':modelo, 'form': pergunta_form})#Só para poder pegar a url de quem ele herda
...
def ver_respostas_modelo(request, id_pergunta):
	...
	return render(request, 'onshop_auto/respostas_modelo.html', {'complemento':modelo, 'modelo':modelo, 'pergunta': pergunta, 'respostas': respostas})
...
def adicionar_resposta_modelo(request, id_pergunta, id_resposta=None):
	...
	return render(request, 'onshop_auto/form_resposta_modelo.html', {'complemento':modelo,'modelo':modelo, 'pergunta': pergunta, 'form': resposta_form})
...
def atribuir_produtos(request, complemento_id):
		...
		if atribuir_form.is_valid():
			...
			for x in atribuidos:
				...
				for pergunta in perguntas_modelo:
					pergunta_atribuida = PerguntaAuto.objects.create(produto=produto, pergunta=pergunta.pergunta, tipo=pergunta.tipo, limite=pergunta.limite)
					respostas_modelo = RespostaModeloAuto.objects.filter(pergunta=pergunta)
					for resposta in respostas_modelo:
						resposta_atribuida = RespostaAuto.objects.create(pergunta=pergunta_atribuida, resposta=resposta, preco_adicional=resposta.preco_adicional)
			#Faz a atribuição
			#TODO: Enviar Mensagem
	...
	return render(request, 'onshop_auto/atribuir_form.html', {'complemento':complemento, 'perguntas':perguntas_modelo, 'form': atribuir_form})
...
def analisa_pedido_auto(request):
	#TODO: Analisar esse trecho de código
	'''
	session_key = request.GET.get('key', None)

	try:
		pedido = get_object_or_404(PedidoAuto, session_key=session_key, status=PedidoAuto.PEDIDO_CARRINHO)
		total = pedido.total
	except:
		response = JsonResponse({'status': 'error', 'product_snippet': 'Blau! Não é possível fazer o pedido nesse momento!'})
		response.status_code = 404
		return response
	'''
	verifica_horario()
...
def ver_garcons(request):
	...
	return render(request, 'onshop_auto/garcons_auto.html', {'garcons': garcons, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens })
...
def editar_garcons(request, id=None):
	...
	return render(request, 'onshop_auto/form_garcom.html', {'form': garcom_form, 'qtde_novos_pedidos': qtde_novos_pedidos, 'qtde_pedidos_andamento': qtde_pedidos_andamento, 'qtde_pedidos_finalizados': qtde_pedidos_finalizados, 'qtde_mensagens': qtde_mensagens })
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			try:
				...
				garcom = get_object_or_404(Garcom, usuario=form.cleaned_data['usuario'], senha=form.cleaned_data['senha'])
				garcom.token = novo_token
				garcom.save()

				return redirect(reverse('onshop_auto:atendimento_mesas', kwargs={'id_garcom': garcom.id, 'token' : garcom.token}))
                
			except Exception as e:
				return render(request, 'onshop_auto/atendimento.html', {'form': form, 'login_error': 'Usuário e/ou senha incorretos'})
...
def atendimento_mesas(request, id_garcom, token, codigo=None): #O código é apenas para quando se clicar em alguma mesa
	...
	if codigo:
		...
		try:
			get_object_or_404(ComandaSession, session_key=request.session.session_key,comanda=comanda)  # Caso já exista a ComandaSession não faz nada
		except:
			request.session.create()  # Cria uma nova sessão
			ComandaSession.objects.create(session_key=request.session.session_key, comanda=comanda)

		return redirect(reverse('onshop_auto:ver_cardapio_garcom', kwargs={'id_comanda': comanda.id, 'id_garcom': garcom.id, 'token': token}))

	mesas = Mesa.objects.all()

	return render(request, 'onshop_auto/atendimento_mesas.html', {'mesas': mesas, 'garcom': garcom, 'token': garcom.token})
...
def ver_cardapio_garcom(request, id_comanda, id_garcom, token):
	...
	return render(request, 'onshop_auto/cardapio.html',
                          {'lista': lista, 'mesa': mesa, 'estabelecimento': estabelecimento, 'quantidade': quantidade,
                           'total': total, 'comanda': id_comanda, 'categorias' :categorias, 'token': token, 'garcom': garcom, 'id_garcom': garcom.id, 'session_key': request.session.session_key})
...
def concluir_pedido_garcom(request, id_comanda, id_garcom, token):
	...
	try:
		comandasession = get_object_or_404(ComandaSession, session_key=request.session.session_key, comanda=comanda, comanda__status=Comanda.COMANDA_ABERTA)
		pedido = get_object_or_404(PedidoAuto, session_key=request.session.session_key, comanda=comanda, status=PedidoAuto.PEDIDO_CARRINHO)
	except:
	...
	return render(request, 'onshop_auto/confirmacao_modal_garcom.html', { 'pedido':pedido, 'estabelecimento': estabelecimento, 'comandasession': comandasession, 'mesa': mesa, 'quantidade': pedido.quantidade_itens, 'total': total, 'id_garcom': garcom.id, 'token': garcom.token})

#-------- Gambiarras e Afins --------------
@login_required
def vai_e_volta(request):
	'''
	Só para forçar o refresh chamado quando se clica em imprimir Novos Pedidos
	'''
	return redirect('onshop_auto:novos_pedidos_auto')