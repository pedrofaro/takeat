...
class ProdutoAuto(models.Model):
	...
	nome = models.CharField(max_length=255)
	categoria = models.ForeignKey(CategoriaAuto)
	descricao = models.TextField(null=True, blank=True)
...
class PerguntaAuto(models.Model):
	...
	produto = models.ForeignKey(ProdutoAuto, null=True, blank=True)
...
class RespostaAuto(models.Model):
	pergunta = models.ForeignKey(PerguntaAuto, null=True, blank=True)
...
class PerguntaModeloAuto(models.Model):
	...
	modelo = models.ForeignKey(ComplementoModeloAuto, null=True, blank=True) #Fica linkado a uma lista de Complementos Modelo
	pergunta = models.CharField(max_length=800)
	tipo = models.IntegerField(choices=RESPOSTAS, default=RESPOSTA_EXCLUSIVA)
	limite = models.IntegerField(null=True, blank=True) #Se houver multipla ou recorrente quantas respostas podem ser dadas
...
class RespostaModeloAuto(models.Model):
	pergunta = models.ForeignKey(PerguntaModeloAuto, null=True, blank=True)
	resposta = models.CharField(max_length=400)
	preco_adicional = models.DecimalField(null=True, blank=True, max_digits=8, decimal_places=2) #Caso a opção acrescente algo no valor
...
class Mesa(models.Model):
	...
	codigo_mesa = models.ForeignKey(Codigo, null=True, blank=True)
...
class PedidoAuto(models.Model):
	...
	garcom = models.ForeignKey(Garcom, blank=True, null=True) #Adicionado a Chave para Garçom
	...
	comprador = models.ForeignKey(CompradorPedidoAuto, blank=True, null=True)
...
class ItemPedidoAuto(models.Model):
	...
	'''
	comanda = models.ForeignKey(Comanda, blank=True, null=True)
	pedido_atrelado = models.ForeignKey(PedidoAuto, blank=True, null=True)
	comprador = models.ForeignKey(CompradorPedidoAuto, blank=True, null=True)
...
class Avaliacao(models.Model):
	numero_mesa = models.CharField(max_length=3, null=True, blank=True)
